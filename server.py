#!/usr/bin/env python3
"""
Nager URL Test 1772121510 MCP Server - FastMCP with D402 Transport Wrapper
...
logger = logging.getLogger('nager-url-test-1772121510_mcp')
...
logger.info("="*80)
logger.info(f"Nager URL Test 1772121510 MCP Server (FastMCP + D402 Wrapper)")
logger.info(f"API: https://date.nager.at")
logger.info(f"Payment: {SERVER_ADDRESS}")
logger.info("="*80)

# Create FastMCP server
mcp = FastMCP("Nager URL Test 1772121510 MCP Server", host="0.0.0.0")
...
# - NAGER_URL_TEST_1772121510_API_KEY: Server's internal API key for payment mode
#
...
def create_app_with_middleware():
    ...
    app.add_middleware(
        ...
        server_name="nager-url-test-1772121510-mcp-server"  # MCP server ID for tracking
    )
    ...
    async def health_check(request: Request) -> JSONResponse:
        ...
        return JSONResponse(
            content={
                "status": "healthy",
                "service": "nager-url-test-1772121510-mcp-server",
...
if __name__ == "__main__":
    logger.info("="*80)
    logger.info(f"Starting Nager URL Test 1772121510 MCP Server")
    logger.info("="*80)
